...
__sbit __at 0xB3 BILED0; // BILED0, associated with Porn 3 Pin 3
...
void Port_Init(void)
{
  //open drain is just input
  //push-pull either input
  //
    // Port 3
    P3MDOUT |= 0xD8; // set Port 3 output pins to push-pull mode (fill in the blank)
    P3MDOUT &= ~0x03; // set Port 3 input pins to open drain mode (fill in the blank)
    P3 |= 0x03; // set Port 3 input pins to high impedance state (fill in the blank)

    // Port 2
    // configure Port 2 as needed
    P2MDOUT &= ~0x01; // set Port 2 input pin to open drain mode.
    P2 |= 0x01 // set Port 2 input pin to high imedance state.
//
//
}
...
void GM2(void)
{
  if (!SS && PB2 && PB1) // if Slide Switch activated (On position), not push buttons pressed
  {
    BILED0 = 1;
    BILED1 = 1;
    Buzzer = 1;
  ...
  else if (SS)           // if Slide Switch not activated
  {
    BILED0 = 0;
    BILED1 = 1;
    Buzzer = 1;
