...
void ExtraCreditTest(){
  std::cout << "TESTING EXTRA CREDIT" << std::endl;
  Matrix a(10,10,10);
  a.set(5,5,5);
  a.set(1,1,1);
...
void StudentTest()
{
  ...
  //checking get row/col

  double* p1 = b.get_row(15);
  if(p1 == NULL)
  {
  ...
  {
    std::cout << "get row not" << std::endl;
  }
  delete[] p1;

  double* p2 = b.get_col(15);
  if(p2 == NULL)
  {
    std::cout << "get col works" << std::endl;
  }
  else
  {
    std::cout << "get col not" << std::endl;
  }
  delete[] p2;
