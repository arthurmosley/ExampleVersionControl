...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    //checks if its already there and assigns false.
    hash_list_itr p = m_table[h].begin();
    for (p; p != m_table[h].end(); p++) {
      if ( *p == key) 
        return std::pair< iterator, bool >(iterator(this, h, p) , false);
    }
    m_table[h].push_back(key);
    p = m_table[h].begin();

    m_size++;
    //if its not in there returns it and says that it is its first time in there.
    return std::pair< iterator, bool >(iterator(this, h, p), true);
  }
