...
// We use a C++ functor (a function wrapped up in a class object) to
// specify the hash function function.  
...
// the hash function is passed as the 2nd template class (we can only
// template over classes, not functions) when we create a hashset
