...
void find_path(Point coordinates, unsigned int& path_numbers, vector<vector<bool> > const& blocked_grid, vector<Point>& path) {
  if (coordinates.x == 0) {
    if (coordinates.y == 0) {
      path_numbers++;
      path[0].x = -999999999;
    }
    else if (blocked_grid[0][coordinates.y]) {
      if (path[0].x != -999999999) {
        path.pop_back();
      }
    }
    else {
      ...
      find_path(coordinates,path_numbers,blocked_grid,path);
      if (path[0].x != -999999999) {
        path.pop_back();
      }
    }
    ...
    if (blocked_grid[coordinates.x][0]) {
      if (path[0].x != -999999999) {
        path.pop_back();
      }
    }
    ...
    if (!blocked_grid[coordinates.x][coordinates.y]) {
      coordinates.x--;
      if (path[0].x != -999999999) {
        path.push_back(coordinates);
      }
...
/*void print_grid2(vector<vector<bool> > const& blocked_grid, int start_x, int start_y) {
    cout << "Here is the grid with the origin in the upper left corner, x increasing \n"
       << "horizontally and y increasing down the screen.  An 'X' represents a blocked\n"
       << "location and the 'S' represents the starting location.\n\n";
        int sx;
...
int main(int argc, char* argv[]) {
  ...
  Point coordinates = Point(start_x,start_y);
  unsigned int path_numbers = 0;
  vector<Point> path;
  path.push_back(coordinates);
  path.push_back(coordinates);
  find_path(coordinates,path_numbers,blocked_grid,path);
  cout << path_numbers << endl;
