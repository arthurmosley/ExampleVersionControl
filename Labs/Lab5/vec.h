...
public:
  ...
  void shrink(size_type n, const T &fill = T());
...
template <class T> void Vec<T>::shrink(size_type n, const T& fill)
{
  m_alloc = n;
  T* new_d = new T[m_alloc];
  for(unsigned int i = 0; i < m_size; i++)
  {
    new_d[i] = m_data[i];
  }
...
template <class T> typename Vec<T>::size_type remove_matching_elements(Vec<T>& v, const T& t) {
  typename Vec<T>::size_type count  = 0;
  int o_size = v.size();
  Vec<T> temporary;
  for (typename Vec<T>::iterator a = v.begin(); a != v.end(); a++) {
    if (*a != t) {
